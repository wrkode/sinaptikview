  ...
  <div class="card p-fluid">
    <div>
        <div class="flex justify-between items-center mb-2">
             <h2 class="text-xl font-semibold">Jobs ({{ selectedNamespace === 'all' ? 'All Namespaces' : selectedNamespace }})</h2>
             <!-- Namespace Dropdown -->
             <Dropdown 
                ...
                :disabled="loadingNamespaces || loadingJobs"
                style="width: 200px;"
             />
        </div>
       <DataTable :value="jobs" :loading="loadingJobs" tableStyle="min-width: 50rem">
         <template #empty> 
            <div v-if="jobError">Error loading jobs: {{ jobError }}</div>
            <div v-else>No jobs found in {{ selectedNamespace === 'all' ? 'all namespaces' : `namespace '${selectedNamespace}'` }}.</div> 
         </template>
...
const fetchJobs = async () => {
  ...
  const url = `/api/v1/namespaces/${ns}/jobs`; 
  ...
  try {
    ...
    if (!response.ok) {
        ...
        console.error("Job fetch failed response:", errorData);
        throw new Error(`HTTP error! status: ${response.status}`);
    }
    const data = await response.json();
...
onMounted(() => {
  fetchNamespaces(); 
  fetchJobs(); // Fetch initial jobs for default namespace
});
</script>
